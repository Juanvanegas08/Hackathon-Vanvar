...
NEW_LEAD_SEQUENCE: tuple[QuestionRule, ...] = (
    ...
    QuestionRule(
        ...
        question="¿Cuánto ganas al mes, más o menos?",
    ...
    QuestionRule(
        field="ingreso_hogar",
        question="¿Cuánto entra al hogar al mes entre todos?",
        type=QuestionFieldType.CURRENCY,
    ...
    QuestionRule(
        field="ahorro",
        question="¿Tienes algo ahorrado para vivienda? ¿Cuánto aproximadamente?",
        type=QuestionFieldType.CURRENCY,
    ...
    QuestionRule(
        ...
        question="¿Cuánto pagas al mes en deudas o cuotas, más o menos?",
        type=QuestionFieldType.CURRENCY,
    ...
    QuestionRule(
        ...
        question="¿Hoy tienes vivienda propia?",
        type=QuestionFieldType.BOOLEAN,
    ...
    QuestionRule(
        ...
        question="¿Cuántas personas tienes a cargo?",
        type=QuestionFieldType.INTEGER,
        reason="Las personas a cargo influencian necesidades de vivienda",
    ),
    QuestionRule(
        field="situacion_crediticia",
        question="¿Cómo te va con el crédito: al día, con atrasos, o sin reportes?",
    ...
    QuestionRule(
        ...
        question="¿En qué zona o municipio te gustaría vivir?",
        type=QuestionFieldType.TEXT,
        reason="La ubicación deseada filtra proyectos compatibles",
    ),
    QuestionRule(
        field="plazo_compra",
        question="¿Para cuándo te ves comprando: ya, en unos meses, o más adelante?",
        type=QuestionFieldType.ENUM,
        reason="El plazo define urgencia y ruta comercial",
    ),
    QuestionRule(
        field="proyecto_interes",
        question="¿Tienes algún proyecto en mente, o prefieres que yo te oriente?",
        type=QuestionFieldType.TEXT,
...
class QuestionService:
    ...
    def list_upcoming_questions(self, lead: Lead, *, limit: int = 2) -> list[NextQuestion]:
        """Return the next N missing/confirmation questions without mutating the lead.

        Used by the phone channel to ask a short batch (2) in one turn.
        """
        if limit <= 0:
            return []
        upcoming: list[NextQuestion] = []
        ...
        for field in lead.fields_to_confirm:
            if lead.is_field_confirmed(field):
                continue
            upcoming.append(self._confirmation_question(lead, field))
            if len(upcoming) >= limit:
                return upcoming[:limit]
        ...
        for rule in sequence:
            ...
            if not self._is_field_missing(lead, rule.field):
                continue
            # Avoid duplicates already added as confirmations.
            if any(q.field == rule.field for q in upcoming):
                continue
            upcoming.append(
                NextQuestion(
                    field=rule.field,
    ...
    def _confirmation_question(self, lead: Lead, field: str) -> NextQuestion:
        ...
        if field == "salario_mensual":
            ...
            question = (
                f"¿Tu salario sigue más o menos en la categoría {category} "
                "que tenemos registrada?"
            )
        elif field == "personas_a_cargo":
            question = "¿Sigue igual cuántas personas tienes a cargo?"
        elif field == "afiliado":
            question = "¿Confirmas que estás afiliado a Colsubsidio?"
        elif field == "ingreso_hogar":
            question = "¿El ingreso del hogar que tenemos sigue más o menos igual?"
        elif field == "ahorro":
            question = "¿El ahorro para vivienda que tenemos sigue más o menos igual?"
        elif field == "obligaciones_mensuales":
            question = "¿Lo que pagas en deudas al mes sigue más o menos igual?"
        elif field == "situacion_crediticia":
            question = "¿Tu situación de crédito sigue igual?"
        else:
            friendly = {
                "tiene_vivienda": "si tienes vivienda propia",
                "personas_hogar": "cuántas personas viven en tu casa",
                "ubicacion_deseada": "la zona donde te gustaría vivir",
                "plazo_compra": "el plazo para comprar",
                "proyecto_interes": "el proyecto de interés",
            }.get(field, "ese dato")
            question = f"¿Confirmas {friendly}?"
        return NextQuestion(
            field=field,
            question=question,
